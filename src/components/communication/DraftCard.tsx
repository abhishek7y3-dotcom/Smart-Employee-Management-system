    ...
    <div className="group rounded-2xl border border-zinc-200/60 bg-white p-4 transition-all duration-300 hover:shadow-md dark:border-zinc-800/60 dark:bg-zinc-950/40 backdrop-blur-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          {onEdit && (
            <button onClick={onEdit} className="rounded-lg p-1.5 text-zinc-400 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-950/30 dark:hover:text-blue-400 transition-colors" title="Edit">
              <Edit3 className="h-3.5 w-3.5" />
            </button>
          )}
          {onSend && (
            <button onClick={onSend} className="rounded-lg p-1.5 text-zinc-400 hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-950/30 dark:hover:text-emerald-400 transition-colors" title="Send">
