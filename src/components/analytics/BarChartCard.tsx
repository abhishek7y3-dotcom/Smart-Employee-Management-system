...
export const BarChartCard: React.FC<BarChartCardProps> = ({ tasks, employees, className = '' }) => {
  ...
  return (
    <div className={`rounded-lg border border-zinc-200 bg-white p-5 shadow-sm transition-colors duration-300 dark:border-zinc-800 dark:bg-zinc-950 ${className}`}>
      <h3 className="text-base font-bold text-zinc-950 dark:text-zinc-50">Team Workload Breakdown</h3>
      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Number of tasks per employee by status.</p>
        ...
        {tasks.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ left: -20, right: 10, top: 10, bottom: 5 }}>
              ...
              <Legend verticalAlign="bottom" height={36} iconType="circle" iconSize={8} formatter={(value) => <span className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">{value === 'todo' ? 'To Do' : value === 'inProgress' ? 'In Progress' : value === 'completed' ? 'Completed' : 'Overdue'}</span>} />
