...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  return (
    <div className={`relative flex-1 ${className}`}>
      ...
      />
      <div className="absolute left-3.5 top-2.5 text-zinc-400 dark:text-zinc-500">
        <Search className="h-4.5 w-4.5" />
      </div>
      {value && (
        <button
          onClick={() => onChange('')}
          className="absolute right-3.5 top-2.5 text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 cursor-pointer"
          title="Clear search"
        >
          <X className="h-4 w-4" />
        </button>
