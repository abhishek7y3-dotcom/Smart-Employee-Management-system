...
export const PieChartCard: React.FC<PieChartCardProps> = ({ tasks, className = '' }) => {
  ...
  return (
    <div className={`rounded-2xl border border-zinc-200/60 bg-white/90 p-5 shadow-sm transition-colors duration-300 dark:border-zinc-800/80 dark:bg-zinc-950/40 backdrop-blur-sm ${className}`}>
      ...
      <p className="mt-1 text-xs text-zinc-450 dark:text-zinc-500">Breakdown of tasks by active status.</p>
        ...
        {data.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Legend verticalAlign="bottom" height={36} iconType="circle" iconSize={8} formatter={(value) => <span className="text-xs font-semibold text-zinc-600 dark:text-zinc-450">{value}</span>} />
            </PieChart>
          </ResponsiveContainer>
        ) : (
